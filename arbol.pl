...
%Arbol - átomo
arbol(Tripletas, Arbol):- findall(Arista, (member(Tripleta, Tripletas), tripleta_a_arista(Tripleta, Arista)), A),
                             append(["digraph{"], A, A1),
                             append(A1, ["} "], A2),
...
idinstancia_a_arboles(IdIns):- encontrar_CGs_dependencias_id(IdIns, _, Deps), 
                               findall(_, (arbol(Deps, Dot), atomic_list_concat(["/home/alejandra/Documents/",
                                                     "tareasDCC/eso/traduccion/arboles/", IdIns, "/", "1", ".dot"], Direccion),
